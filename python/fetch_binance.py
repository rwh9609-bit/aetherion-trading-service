...
async def fetch_binance_price_async(session, symbol="BTCUSDT"):
    """Fetch current price from either Binance or Coinbase API asynchronously."""
    supported = {"BTC", "ETH", "SOL", "ILV"}
    base = symbol.replace("USDT", "").replace("USD", "")
    base = base.upper()
    if base not in supported:
    ...
    try:
        ...
        async with session.get(cb_url) as resp:
            if resp.status != 200:
                print(f"Coinbase API response for {coinbase_symbol}: {await resp.text()}")
            resp.raise_for_status()
        ...
        try:
            ...
            async with session.get(url, headers=headers) as resp:
                if resp.status != 200:
                    print(f"Binance API response for {binance_symbol}: {await resp.text()}")
                resp.raise_for_status()
                data = await resp.json()
                price = float(data["price"])
                print(f"Successfully fetched price for {base} from Binance")
