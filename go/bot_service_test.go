...
func TestBotServiceLifecycle(t *testing.T) {
	...
	if err != nil || !resp.Success || resp.Id == "" {
		t.Fatalf("create bot failed: resp=%v err=%v", resp, err)
	}
	...
	if st.Parameters["strategy_id"] == "" {
		t.Fatalf("expected strategy_id parameter set")
	}
	...
	reg2 := newBotRegistry()
	if len(reg2.bots) != 1 {
		t.Fatalf("expected persisted bot in new registry, got %d", len(reg2.bots))
	}
}
