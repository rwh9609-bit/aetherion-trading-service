...
func TestGetPrice(t *testing.T) {
	...
	if resp.Price != 54321.12 {
		t.Fatalf("expected price 54321.12 got %v", resp.Price)
	}
}
...
func TestGetPortfolioCreatesDefault(t *testing.T) {
	...
	if p.TotalValueUsd == 0 {
		t.Fatalf("expected default total value")
	}
	...
	if p2.TotalValueUsd != p.TotalValueUsd {
		t.Fatalf("expected same portfolio on subsequent call")
	}
}
