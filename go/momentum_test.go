...
func addHist(s *tradingServer, sym string, times []time.Time, prices []float64) {
	...
	for i := range times {
		arr[i] = histPoint{ts: times[i], price: prices[i]}
	}
	s.priceHist[sym] = arr
	s.histMu.Unlock()
	// also set lastPrices so empty symbol request picks it up
	s.priceMu.Lock()
	s.lastPrices[sym] = prices[len(prices)-1]
	s.priceMu.Unlock()
}
...
func TestGetMomentumBasicAndSorting(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("GetMomentum error: %v", err)
	}
	...
	for _, m := range resp.Metrics {
		if m.Symbol == "A-USD" {
			a = m
		} else if m.Symbol == "B-USD" {
			b = m
		}
	}
	if a == nil || b == nil {
		t.Fatalf("missing metrics for symbols: %+v", resp.Metrics)
	}
	if a.LastPrice != 101 {
		t.Fatalf("expected A last price 101 got %v", a.LastPrice)
	}
	if a.PctChange_5M <= 0 {
		t.Fatalf("expected positive 5m change for A")
	}
	if b.PctChange_5M != 0 {
		t.Fatalf("expected zero 5m change for B")
	}
}
...
func TestGetMomentumDefaultSymbols(t *testing.T) {
	...
	if resp.Metrics[0].Symbol != "C-USD" {
		t.Fatalf("expected C-USD symbol got %s", resp.Metrics[0].Symbol)
	}
}
