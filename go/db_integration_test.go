...
func TestPostgresUserAndBotIntegration(t *testing.T) {
	if os.Getenv("RUN_DB_TESTS") != "1" {
		t.Skip("set RUN_DB_TESTS=1 to run integration test")
	}
	...
	if err != nil {
		t.Skipf("unable to start postgres container (skipping): %v", err)
	}
	...
	if err != nil || !rresp.Success {
		t.Fatalf("register failed: %v %v", rresp, err)
	}
	lresp, err := auth.Login(ctx, &pb.AuthRequest{Username: "alice", Password: "pw"})
	if err != nil || !lresp.Success || lresp.Token == "" {
		t.Fatalf("login failed: %v %v", lresp, err)
	}
	...
	if err != nil || !cResp.Success {
		t.Fatalf("create bot failed: %v %v", cResp, err)
	}
